...
3. RPC 压力测试：用真实状态读取方法持续请求 Chainstack、公开 fallback，统计成功数、失败类型、p50/p95/max latency。
...
npm run rpc:stress -- --duration-ms 30000 --concurrency 4
```
...
- `rpc:stress` 中 Chainstack p95 不应明显超过数秒，失败率应接近 0。
